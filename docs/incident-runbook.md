...
| `E_BUREAU_ORPHAN_PRESERVED` | 入力と照合できない局別行を確認する。差分同期はその行を削除せず保持する。チェック初期値だけの未使用行は対象外。過去の内容が空の警告は、局全体に未命名の実データ行がないことを再確認できた場合のみ理由付きで自動解消する。古い行番号だけで別企画を削除しない |
| `E_MANUAL_REVIEW_RESOLUTION_FAILED` | 空行警告の完了更新に失敗。他行は継続し、回答内容をログに含めない。書き込み権限などを確認後、局別差分同期で再試行する。理由追記だけ済んだ行も再確認して完了できる |
