...
本番から取り込んだ画像処理はDrive v3・Driveアクセス・Googleサムネイル取得の権限を保持する。詳細は[画像処理の管理](image-processing.md)を参照する。原本への書き込みと共有範囲の拡大は行わず、画像用フォルダIDはScript Propertiesだけに設定する。

その他掲載情報の重複回答を採用する権限は、原本と局別タブの手動入力を確認できる運用担当者に限定する。`その他掲載情報: 選択した回答を採用`は、確認画面で承認した対象企画の局別行を更新・移動し、同名の他回答の識別子を非表示の`26局別回答除外管理`へ保存する。非表示はアクセス制御ではないため、Spreadsheetの編集者を適切に限定する。台帳は通常直接編集せず、誤った除外の取消しは権限者が対象識別子・出力への影響を確認して行う。追加のOAuthスコープや外部通信は使用しない。
