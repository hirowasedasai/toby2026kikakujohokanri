...
入力5タブにはフォーム連携以外の書き込みを行わない。マスター編集権限、処理ログ閲覧権限、Script Properties・trigger変更権限は少人数に限定する。`参参一覧`はメールアドレスを含む内部提出管理表であり、公開用出力ではない。参参運用担当者だけに共有し、運用担当者が通常変更するのは`提出状況`だけとする。フォーム回答・確定版・照合列・回答識別子は直接編集しない。誤回答の削除権限は原本を確認できる担当者に限定し、通常の行削除ではなく`参参: 選択行を誤回答として除外`を使う。非表示の`26参参除外管理`は回答識別子だけを保存し、通常は直接編集しない。局別タブと`26要手動確認`はメールを含まないが、担当者名、自由記述、添付リンクを含むため、該当する局と運用担当者に閲覧範囲を限定する。局別タブでは手動6列を運用担当者の編集対象とし、通常企画の自動12列はフォーム原典・差分同期結果として直接編集しない。その他掲載情報だけはフォームにない列への手動補完を許可する。要手動確認の`対応状況`は権限者だけが変更する。
