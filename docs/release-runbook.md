...
8. 環境情報、事前チェック、dry runを実行し、件数確認後に同期する。参参一覧で全フォーム回答が保持され、人が付けた`キャンセル`・`確認中`が保持されることを確認する。合成データを使えるstagingでは、同一メアド・参参名の全行が警告され、誤回答の選択除外後に誤回答が復活せず残りの警告が解除されることも確認する。productionへ検証用回答を追加しない。
