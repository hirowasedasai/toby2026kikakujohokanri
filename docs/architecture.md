...
参参一覧の照合キーは正式企画IDではなく、正規化した`メールアドレス + 参加企画`とする。フォーム回答が一意なら参参名と企画名を回答列・確定版へ反映する。一覧内または回答内の重複は`照合結果`とサニタイズ済みログへ送り、曖昧な回答を自動採用しない。フォーム回答がない既存行と、人が設定した`確認中`・`キャンセル`は保持する。
