...
Apps Script/Sheetsには複数シートをまとめたACID transactionがない。このため`runAll`は同一lock内で同期、参参一覧、屋台情報まとめ、局別9タブ、要手動確認の順に処理し、各段階をログに残す。変更申請は全項目を検証してからメモリ上の企画情報へ原子的に適用し、一部変更だけを書かない。途中失敗時は入力を変更せず、正常だった直前状態を保持する。
