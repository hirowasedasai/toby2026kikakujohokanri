...
同期は`LockService.getScriptLock()`で直列化する。全タブと必須ヘッダー、環境一致を開始前に確認し、致命的な不整合時にはマスターと出力を変更しない。入力は常に`getValues()`で一括取得する。参参一覧と屋台情報まとめはメモリ上で完成させてから一括反映する。局別9タブは既存行と希望状態を企画名で比較し、新規・内容変更・所属局移動の行だけを書き込む。照合できない既存行は自動削除しない。
