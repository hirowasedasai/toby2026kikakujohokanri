...
同期は`LockService.getScriptLock()`で直列化する。全タブと必須ヘッダー、環境一致を開始前に確認し、致命的な不整合時にはマスターと出力を変更しない。入力は常に`getValues()`で一括取得する。屋台情報まとめはメモリ上で完成させてから一括反映する。参参一覧は申込情報を保持し、提出状況と自動6列だけを差分更新して既存行を削除しない。局別9タブも既存行と希望状態を企画名で比較し、新規・内容変更・所属局移動の行だけを書き込む。照合できない既存行は自動削除しない。
...
参参一覧の照合キーは正式企画IDではなく、前年運用と同じく正規化した`メールアドレス + 参加企画`とする。申込台帳の`参加企画`、`メールアドレス`、`参参名・参加申し込み時`は上書きしない。フォーム回答が一意なら回答値と確定版へ反映し、名称差異・申込情報なし・重複は`照合結果`とサニタイズ済みログへ送り、曖昧な回答を自動採用しない。
