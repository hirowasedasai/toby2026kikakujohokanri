...
| 唯一の正本 | `26掲載情報マスター` |
| 差分同期型の提出管理 | `参参一覧`。1回答1行で保持し、同一メアド・参参名だけを警告 |
...
- `参参一覧`はフォーム回答を1回答1行で保持し、参参名をそのまま確定版へ反映する。正規化後のメールアドレスと参参名が両方同じ回答だけを全件`確認中`にし、黄色で警告する。企画名、参加企画、回答日時が違っても最新1件へ自動統合しない。人が誤回答の行を選択して専用メニューから除外すると、非PIIの回答識別子を台帳へ残すため次回同期でも復活しない。同期後は行全体を`参加企画`、`企画名・確定版`、`参参名・確定版`の順に並べ替え、手動の提出状況を同じ行のまま保持する。
