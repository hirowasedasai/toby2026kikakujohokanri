...
入力4タブは人が用意します。スクリプトは入力タブを自動作成しません。メニューの`出力スキーマを更新`は、stagingでは即時、productionでは確認文字列`PRODUCTION`を要求して、マスター・出力・ログのスキーマだけを作成または移行します。通常同期はスキーマを作りません。
...
- 同じ正式IDが通常提出2タブにある場合は同一企画とみなし、同じタブ内では後の行を採用する。正式IDがない重複候補は原則として自動統合しない。ただし、同じ入力タブ内で画像リンクだけが異なり、各回答日時から新旧を一意に決められる再送は最新回答を採用する。
- 更新入力の空の所属局・販売物・画像リンクは既存値を消さない。削除が必要な場合は権限者がマスターで確認して明示的に修正する。
- `TMP-`は暫定管理ID専用の予約prefix。暫定キー衝突時は最初の1件だけを要確認として残し、後続を自動統合しない。画像リンクだけを更新した時系列の明確な再送は例外として最新回答へ更新し、処理ログへINFOを残す。
- 参加企画・団体名・企画名が一致する別暫定キーはメールアドレス変更の疑いとして、新旧双方を要確認にし、自動統合しない。同じメールアドレスでも企画名が異なる回答は別企画として扱う。
- `参参一覧`は正規化したメールアドレス、参加企画、企画名でフォーム回答を一意に管理し、参参名をそのまま確定版へ反映する。同じ3項目の一覧内または回答内の重複は原則`確認中`にし、一意でない回答を自動採用しない。画像リンクだけを更新した時系列の明確な再送は最新回答を採用して`提出済み`へ戻す。同期後は行全体を`参加企画`、`企画名・確定版`、`参参名・確定版`の順に並べ替え、手動の提出状況を同じ行のまま保持する。
