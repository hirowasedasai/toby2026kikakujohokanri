...
Googleフォーム回答を読み取り専用の生データとして保ち、`26掲載情報マスター`を掲載情報の唯一の正本にするcontainer-bound Google Apps Scriptです。`参参一覧`は申込台帳とフォーム回答を差分照合し、`屋台情報まとめ`はマスターから生成します。処理の要約とサニタイズ済みエラーは`26処理ログ`へ残します。
...
| 唯一の正本 | `26掲載情報マスター` |
| 差分同期型の提出管理 | `参参一覧`。申込情報3列を保持してフォーム回答を照合 |
| 再生成出力 | `屋台情報まとめ` |
...
- `参参一覧`は正規化したメールアドレスと参加企画で申込行とフォーム回答を照合する。参参名差異、申込情報なし、一覧内または回答内の重複は`確認中`にし、一意でない回答を自動採用しない。
