...
| 唯一の正本 | `26掲載情報マスター` |
| 差分同期型の提出管理 | `参参一覧`。1回答1行で保持し、同一メアド・参参名・参加企画だけを警告 |
| 誤回答の復活防止 | 選択行の非PII識別子を非表示の`26参参除外管理`へ登録 |
| 再生成出力 | `屋台情報まとめ` |
...
入力5タブは人が用意します。スクリプトは入力タブを自動作成しません。メニューの`出力スキーマを更新`は、stagingでは即時、productionでは確認文字列`PRODUCTION`を要求して、マスター・出力・非表示の参参除外管理・ログのスキーマだけを作成または移行します。通常同期はスキーマを作りません。

操作の順序は[運用フロー](docs/operation-flow.md)、production反映は[リリース手順](docs/release-runbook.md)を参照してください。

## メニューとトリガー

スプレッドシートを開くと`当日情報媒体AI`メニューが表示されます。事前チェック、ドライラン、同期、参参一覧の差分同期、参参誤回答の選択除外、屋台情報更新、スキーマ移行、自動同期の有効化・無効化、ログ表示、環境表示を実行できます。
...
- `参参一覧`はフォーム回答を1回答1行で保持し、参参名をそのまま確定版へ反映する。正規化後のメールアドレス、参参名、参加企画がすべて同じ回答だけを全件`確認中`にし、黄色で警告する。参加企画が違う回答は別企画として扱い、警告しない。同じ参加企画内では企画名や回答日時が違っても最新1件へ自動統合しない。人が誤回答の行を選択して専用メニューから除外すると、非PIIの回答識別子を台帳へ残すため次回同期でも復活しない。同期後は行全体を`参加企画`、`企画名・確定版`、`参参名・確定版`の順に並べ替え、手動の提出状況を同じ行のまま保持する。
